...
#        self.profiler = Profiler()
    ...
    def examine(self, model_points, t=0, level=0):
        '''
    ...
    def get_best_match(self, model, points, t=0):
        '''
        ...
        best_match = points[0, :]
        point_profile = model.profile(self.image, points)
        distance = model.evaluate(point_profile)
        ...
        for i in range(len(points_along_normal)):
            ...
            profile = model.profile(self.image, normPoints)

            # evaluate the profile and choose the optimal one
            new_distance = model.evaluate(profile)
